 ...
 * and returns it WITHOUT listening, so tests drive it via `app.inject()` and the bootstrap (index.ts)
 * owns network/lifecycle. Access tokens are minted by identity (`iss=identity`, `aud=vip`); the
 * evidence service verifies with those expectations (matching the gateway).
 */
...
export async function buildServer(opts: BuildServerOptions): Promise<BuiltServer> {
  ...
  registerEvidenceRoutes(app, { service: opts.service, auth });
