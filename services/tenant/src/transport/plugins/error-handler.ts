...
export function registerErrorHandler(app: FastifyInstance): void {
  app.setErrorHandler((error: FastifyError, request, reply) => {
    ...
    // A tenancy violation is always a fail-closed 403 with an opaque message — never leak
    // which tenant/record was targeted. The real reason is logged server-side.
