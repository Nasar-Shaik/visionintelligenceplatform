...
- **SLO-based alerting** (error budgets) rather than raw-threshold noise; multi-window burn-rate alerts. Alerts route to on-call (paging), with runbooks linked ([docs/runbooks](../runbooks/)). Separate from _customer_ event alerts (those are Notifications, [11](11-WORKFLOW-ENGINE.md)).
