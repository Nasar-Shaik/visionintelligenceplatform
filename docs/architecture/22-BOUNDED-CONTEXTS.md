...
- **Extension points:** tracker algorithms, re-ID models (via `model.provider`). **Future:** global city-scale re-ID. _(Called out as its own context because so many compositions depend on it — see [Capability Dependency Graph, 05](05-CAPABILITY-ARCHITECTURE.md).)_
