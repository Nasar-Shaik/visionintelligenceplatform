...
- A **composition** answers "what is happening" as a **reusable business measure** (12 people in queue; occupancy 84%; perimeter breached) — **without** deciding whether it _matters_ (that is a Rule) or _to whom_ (that is a Workflow/Pack).
...
- **Supermarket checkout SLA:** enable **Queue Analytics** composition → it emits `composition.queue.wait-exceeded` → tenant rule _IF wait-exceeded in `checkout` THEN notify store manager_ → Retail Pack ships that rule + a queue dashboard. No new capability, no new composition code — just enablement + a rule.
