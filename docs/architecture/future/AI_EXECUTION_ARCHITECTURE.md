...
| --- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Video Source**        | Own the RTSP/MP4 stream + credentials + reconnect                                                                                 | source → frames (raw)                               | **exists** (Media `StreamSupervisor`, G-2) + **exists in the AI tier (AI-5b)** — `StreamSource` is a transport-neutral Protocol (RTSP/HTTP/USB/file/WebRTC/ONVIF/cloud select by _declared type_), with `ConnectionSupervisor` owning lifecycle, bounded-backoff reconnect, availability + recovery, and credential redaction                                                                                                                                                                                                                                                                                                                                                                                       |
| 2   | **Frame Decoder**       | Decode container/codec → raw frames (ffmpeg/OpenCV)                                                                               | bytes → `Frame(ts, bytes)`                          | **exists** — `StubFrameDecoder`/`OpenCvFrameDecoder` (AI-1) for finite sources; `OpenCvStreamSource` (AI-5b) for live ones. `Frame` is frozen/immutable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 3   | **Frame Sampler**       | Down-sample to a target FPS / keyframe stride; drop under load                                                                    | frames → sampled frames                             | **exists** — `FrameSampler.sample()` (batch, AI-1) + `.admit()` (streaming, AI-5b), one owner of sampling policy. Sampling = `framesSkipped` (policy); bounded-queue overflow = `framesDropped` (degradation) — never conflated                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 4   | **Inference Scheduler** | Batch/queue frames to the runtime; backpressure; per-camera fairness                                                              | frames → inference requests                         | **exists (AI-5b + AI-5c)** — `StreamPipeline` owns the per-session bounded queue + drop policy; `InferenceScheduler` owns **cross-camera fairness** (weighted-fair, structurally starvation-free), **admission control** (typed refusals + capacity reservation for critical/recovery work), **batching**, and a **`ResourceGovernor`** walking an ordered, reversible, hysteretic degradation ladder (`reduced-fps → reduced-resolution → reduced-behaviors → shedding-frames → suspended`) with **predictive** triggering. Compute is abstracted by `ComputeResource` (cpu/cuda/tensorrt/openvino/metal/tpu/npu, abstract units, node-qualified ids), so scheduling is hardware-independent and distributed-ready |
...
| 7   | **Behaviour Engine**    | Windowed analysis over tracks/zones (loiter/queue/intrusion/fire/crowd/occupancy) + **composite** behaviours over BehaviorResults | tracks → `BehaviorResult[]` → `CompositeBehavior[]` | **exists (AI-3 + AI-4)** — `BehaviorAnalyzer`+`BehaviorRegistry` over immutable `BehaviorContext`; `TemporalWindow`; `CompositeBehaviorAnalyzer`+`CompositeRegistry` (config-driven, cycle-safe); `BehaviorProfile` deployment config; `BehaviorResultTranslator`; fully swappable                                                                                                                                                                                                                                                                                                                                                                                                                                  |
