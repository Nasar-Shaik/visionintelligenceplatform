...
| 7   | **Behaviour Engine** | Stateful, windowed analysis over tracks/zones (loiter, queue, intrusion, fire) | tracks → `BehaviorResult[]`  | **exists (AI-3)** — `BehaviorAnalyzer`+`BehaviorRegistry` over immutable `BehaviorContext`; `TemporalWindow`; `BehaviorResultTranslator`; behaviours fully swappable |
