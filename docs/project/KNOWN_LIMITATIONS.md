...
|                       |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Current behaviour** | There is no per-camera AI assignment: no profile, no model selection, no priority, no cost estimate, no last-inference time, no inference health — none of these exists in any contract, route or store. ⚠️ **Updated 2026-08-05 (P-8 Phase 2): frames now reach the runtime** — measured at 16 cameras, 32 fps, every frame accounted for. But the runtime runs the **`stub` backend**, which returns a fabricated result and has no model registered, so **nothing is actually analysed yet**. The transport is real; the perception is not |
| **Customer impact**   | ⚠️ **Say this before anyone assumes selective AI is a configuration exercise.** A customer who expects to enable analysis on 20 of 500 cameras is describing a milestone, not a screen. The camera page states it per camera rather than leaving the absence to be discovered                                                                                                                                                                                                                                                                 |
| **Planned**           | **P-8**, whose stated dependency is exactly this frame path (TD-4/TD-5). It needs an assignment contract, a store, an enforcement point in the pipeline and a cost model. ⚠️ Building the UI first would be a control that configures nothing. Designed in 🔒 [SELECTIVE_AI_PROCESSING](../architecture/future/SELECTIVE_AI_PROCESSING.md). **Phase 1 landed 2026-08-05** — the runtime now runs in the production stack, connected to nothing: it changes what is possible next and changes nothing a customer can see                       |
