...
| **Also**              | ⚠️ **Identity fragments as a host saturates.** Measured on the tracking ladder: one walking person per camera returned exactly one identity up to 4 cameras, 11 for 8, and **21 for 16** — a ~31 % overhead. Fewer analysed frames per camera means a bigger gap between observations, which is a harder association; past the engine's tolerance it correctly starts a new identity rather than guessing a wrong one. ⚠️ Fragmentation, **not** swapping — the two are different failures and only the second is dangerous |
