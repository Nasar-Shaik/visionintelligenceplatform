...
| **Planned**           | Nothing. Exactly-once across a broker boundary requires transactional delivery the backbone does not offer, and claiming it would be worse than documenting the window. The windows are configurable per deployment                                                                                                                                                                                                                                                                |
...
| **Planned**           | A disk-backed spill queue would trade the loss for bounded disk, and is not built. ⚠️ It is a real feature, not a setting: it needs its own retention, its own replay ordering and its own failure mode. Deferred until a customer says lost events during an outage matter more than the simplicity                                                                                                                                                    |
