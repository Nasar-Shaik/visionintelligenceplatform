...
- `2026-07-29` Slice 12 (P1-7): **zero new dependencies** — `@vip/service-rules` reuses the existing fastify/mongodb/helmet/prom-client + `@vip/messaging`/`@vip/tenancy`/`@vip/contracts` set; the sandboxed rule DSL is a hand-written pure interpreter (no CEL/JS-sandbox library — avoids arbitrary-execution + ReDoS surface). Windowed rule state is in-process (a Redis client is deferred, [TD-7](../../tracking/TECH-DEBT.md)). `@vip/contracts` rules/incident schemas + `EventEnvelope` evolution (`envelopeVersion`/`category`) add no packages. 35 rules tests (incl. 2 real-Mongo) + 4 new contract tests; full gate green (327 TS + 40 Python, import-graph 15 pkgs / 0 violations, contracts 25).
- `2026-07-29` Slice 13 (P1-8): **zero new dependencies** — `@vip/service-workflow` + `@vip/service-notify` both reuse the existing fastify/mongodb/helmet/prom-client + `@vip/messaging`/`@vip/tenancy`/`@vip/permissions`/`@vip/contracts` set; the webhook `ChannelSender` uses the Node global `fetch` (no HTTP-client dep). `@vip/contracts` incident + notification schemas add no packages. 37 new TS tests (19 workflow incl. 3 real-Mongo, 18 notify incl. 2 real-Mongo) + 8 new contract tests; full gate green (30 contract schemas, import-graph 17 pkgs / 0 violations).
- `2026-07-28` Slice 11 (P1-5): added the **`@nats-io` v3 modular client** for `@vip/messaging` — `@nats-io/jetstream 3.4.0` + `@nats-io/transport-node 3.4.0` (pure-JS; transitive `@nats-io/nats-core`, `@nats-io/nkeys`, `@nats-io/nuid`; **no `postinstall`/`install` scripts** — verified against the pnpm supply-chain policy). Chosen over the **deprecated** `nats` v2 monolith ("Package moved. Use @nats-io/transport-node"). `@vip/service-events` reuses the existing fastify/mongodb/helmet/prom-client set. For the inference→backbone sink, `nats-py==2.15.0` (pure-Python asyncio) is pinned in `ai/inference/requirements.txt` — **only** for `INFERENCE_EVENT_SINK=nats` (integration/production); the default `null` sink + the stdlib unit suite need none of it. 29 new TS tests (10 messaging + 19 events, incl. 2 real-Mongo) + 2 Python config tests; full gate green.
- `2026-07-29` P2-1.0 (Operations Console foundation): first **frontend** dependencies — all scoped to `apps/console` only (the new `app` import-graph layer). Runtime: `react`/`react-dom` 19, `react-router-dom` 7, `@reduxjs/toolkit` 2 + `react-redux` 9 (client state), `@tanstack/react-query` 5 (server state), `lucide-react`, `clsx` + `tailwind-merge` + `class-variance-authority`, `zod` 4 (shared with contracts). Build/test: `vite` 6 + `@vitejs/plugin-react`, `tailwindcss` 4 + `@tailwindcss/vite` (no PostCSS/config file — v4 `@theme` in CSS), `vitest` 4 + `@testing-library/react`/`jest-dom`/`user-event`, `jsdom`, `msw` 2 (gateway mocks; browser-worker postinstall disabled — node-only in tests), `globals` + `eslint-plugin-react-hooks`/`react-refresh`. Types come from `@vip/contracts` (workspace). Full console gate green.
- `2026-07-29` P2-1.1 (Design System): added (in `apps/console`) **10 Radix UI primitives** (`@radix-ui/react-{dialog,dropdown-menu,label,scroll-area,select,separator,slot,switch,tabs,tooltip}` — pure-JS, accessibility layer under shadcn/ui), `sonner` (toasts), and `recharts` 2 (MetricCard sparklines). No animation library — `data-state` transitions are CSS keyframes. Design-system CSS ≈ 37 kB (gz 7.3). 13 console tests green; import-graph 18 pkgs / 0 violations.
- `2026-07-29` P2-1.2 (Auth): added (in `apps/console`) `react-hook-form` 7 + `@hookform/resolvers` **5** (v5 is standard-schema based and **Zod 4-native** — v3.10 predated Zod 4 and threw the ZodError instead of populating RHF errors), and the **shared** workspace dep `@vip/permissions` (browser-safe — no node imports; used to expand roles → permission patterns for deny-by-default UI gating with the same wildcard PDP the services use). Access token in-memory only; rotating refresh token in localStorage (a future httpOnly-cookie refresh flow is a noted hardening). 18 console tests green; import-graph 18 pkgs / 0 violations (app→shared honoured).
