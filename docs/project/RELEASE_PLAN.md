...
| 0   | ⚠️ **Deployment integrity** — runs **before gate 1**    | `deployment-integrity.mjs` — every service, package, the bundle the edge serves, the image each container runs, and the tools that are not services, against a build of the tree. The commit hash goes in the report |
| 13  | ⚠️ **The verification is itself verified**              | Every script introduced by the release has failed once for the correct reason and been restored: [VERIFICATION_MATRIX](../review/p6/VERIFICATION_MATRIX.md)                                                          |

⚠️ **Gate 0 is numbered zero because it decides whether gates 1–12 mean anything.** The P-6.5 freeze
found a demonstration reset restoring data the milestone had removed: correct source, green tests,
green scripts, and an image built before the fix. Nothing else on this list would have caught it.

⚠️ **Gate 13** was added at the same freeze. A green check and a check that measures nothing are
indistinguishable from the outside — three of the P-6.5 scripts contained a check that could not fail
and two contained a check that could not pass, and the only way anyone found out was by breaking the
product on purpose and watching what happened.
