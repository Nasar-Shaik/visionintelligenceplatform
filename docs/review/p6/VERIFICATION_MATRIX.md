...
| `soak.mjs`                   | Forty minutes open through three restarts: duplicates, rendered order, count against the server, heap, live path                                                        | ✅         | ✅               | browser + `docker`    | ~40 min | non-zero exit; prints transitions and samples      | queue served oldest-first → **5d stayed green**: the console re-sorts client-side (see _What a mutation could not reach_). Two of its own defects were found and fixed instead      | ✅             |
