...
console.log(
  await p.evaluate(() => {
    ...
    document
      ...
      .forEach((s, i) =>
        out.push(
          `hidden select[${i}] value=${JSON.stringify(s.value)} options=[${[...s.options].map((o) => o.value).join(',')}]`,
        ),
      );
    document
      ...
      .forEach((btn, i) =>
        out.push(
          `combobox[${i}] text="${btn.textContent.trim()}" data-placeholder=${btn.hasAttribute('data-placeholder')} aria-expanded=${btn.getAttribute('aria-expanded')}`,
        ),
      );
