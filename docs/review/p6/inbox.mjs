...
const OTHER = { tenant: 'tnt_demo_warehouse', email: 'site.manager@meridian.demo', password: 'Vip-Demo-2026!' };
...
const waiting = await list('?acknowledged=false&limit=200');
const handled = await list('?acknowledged=true&limit=200');

check(all.status === 200, '1a · the delivery log loads', `${all.json?.data?.items?.length} records`);
check(
  waiting.json.data.items.length + handled.json.data.items.length ===
    all.json.data.items.length,
  '1b · ⚠️ the two halves add up to the whole — nothing falls between them',
  `${waiting.json.data.items.length} + ${handled.json.data.items.length} = ${all.json.data.items.length}`,
);
check(
...
const failed = all.json.data.items.filter((n) => n.status === 'failed');
...
check(
  ...
  '2c · with the attempt count, so a one-off is distinguishable from a broken channel',
