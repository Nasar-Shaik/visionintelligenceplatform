...
- **Next step:** the enabler-free console slices (Dashboard · Events · Rules · Incidents · Alerts) are **all code-complete** (⏳ awaiting Architect review). The flagship **Cameras/Live/Analysis/Evidence** slices are next but wait on backend enablers **G-1…G-6** (service extensions — OPERATIONS_CONSOLE §13); those must be built (as reviewed extensions, not new services) before their UI slices. Also open: route-level code-splitting (bundle ≈ 352 kB gz).
