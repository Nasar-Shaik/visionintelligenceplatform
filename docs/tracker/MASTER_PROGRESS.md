...
- **P2-2 — Backend Enablers (G-1…G-6):** **AUTHORIZED & IN PROGRESS** (Architect direction 2026-07-30). Sequential; each G is an independent Architect-review milestone. **G-1 Camera Service Enhancements ✅ ACCEPTED** (Architect review 2026-07-30 — five closing questions answered and aligned: failed-validation model is `200 + checks[]`; capabilities persisted with derived fallback; `/health/check` persists a health snapshot; enable/disable mutates state only, downstream reacts async; permissions confirmed read=all roles, update=owner/admin). Service extension adding camera **capabilities**, **metadata**, **validation/test-connection**, active **health-check**, and **enable/disable**. No new service, no frozen doc touched. **G-2 Media Service Enhancements ✅ code-complete, ⏳ awaiting review** — service extension adding a **Mongo-backed media catalog** (`recordings`, `clips`), **stream health** derivation, and **signed-URL playback**: contracts (+5 published schemas → 35) + domain + ports/in-memory/Mongo adapters + `MediaCatalogService` + supervisor `RecordingSink` seam + routes; all gates green, real-Mongo catalog integration green. No new service, no frozen doc touched. Next after review: **G-3 Inference Runtime**.
- **Next step:** hold for Architect review of **G-2**; do **not** start G-3 until G-2 is accepted (sequential enabler milestones). Also open (console hardening, later): route-level code-splitting (bundle ≈ 352 kB gz), automated browser E2E.
...
| 2     | Productization       | 🟡 In progress | P2-1 Operations Console **enabler-free scope ✅ ACCEPTED** ([acceptance](P2-1-ACCEPTANCE.md)). **P2-2 Backend Enablers in progress** — **G-1 Camera ✅ ACCEPTED**; **G-2 Media ✅ code-complete ⏳ review**; G-3…G-6 pending. Then remaining P2-1 UI slices |
