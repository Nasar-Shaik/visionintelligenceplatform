...
- **P2-2 — Backend Enablers (G-1…G-6):** **AUTHORIZED & IN PROGRESS** (Architect direction 2026-07-30). Sequential; each G is an independent Architect-review milestone. **G-1 Camera Service Enhancements ✅ code-complete, ⏳ awaiting review** — service extension adding camera **capabilities**, **metadata**, **validation/test-connection**, active **health-check**, and **enable/disable** (contracts + service + routes + tests + docs; +22 tests; all gates green; camera integration green on real Mongo). No new service, no frozen doc touched. Next after review: **G-2 Media Service**.
- **Next step:** hold for Architect review of **G-1**; do **not** start G-2 until G-1 is accepted (sequential enabler milestones). Also open (console hardening, later): route-level code-splitting (bundle ≈ 352 kB gz), automated browser E2E.
...
| 2     | Productization       | 🟡 In progress | P2-1 Operations Console **enabler-free scope ✅ ACCEPTED** ([acceptance](P2-1-ACCEPTANCE.md)). **P2-2 Backend Enablers in progress** — **G-1 Camera ✅ code-complete ⏳ review**; G-2…G-6 pending. Then remaining P2-1 UI slices |
