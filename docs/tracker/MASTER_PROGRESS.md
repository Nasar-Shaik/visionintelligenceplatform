...
- **Next step:** **P2-2 — Console Enablers (G-1…G-6)** recommended as the next milestone (acceptance §10) — reviewed backend service extensions that unblock the remaining P2-1 UI slices (Cameras/Live/Analysis/Evidence/Health/Settings/Demo). Sequence: G-1…G-3 (camera/media/inference surfaces) → G-5 (gateway CORS/SSE) → G-4/G-6 (evidence packaging, incident assignment). Each enabler goes for Architect review before its dependent UI slice. Also open (console hardening): route-level code-splitting (bundle ≈ 352 kB gz), automated browser E2E.
...
| 2     | Productization       | 🟡 In progress | P2-1 Operations Console **enabler-free scope ✅ ACCEPTED** (9 slices + DX hardening; [acceptance](P2-1-ACCEPTANCE.md)). Remaining P2-1 slices (Cameras/Live/Analysis/Evidence) + enablers G-1…G-6 pending |
