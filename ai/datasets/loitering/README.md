...
A subject dwelling in an area substantially longer than the passing traffic around them.

## What the runtime produces today

`BehaviorResult` of type `loitering` plus `behavior.loitering.detected` events.

Event types: `behavior.loitering.detected`.

## The honest gap

Dwell thresholds that work in a car park (minutes) are absurd at a till (seconds). Cases must record the threshold they assume.
...
1. Put the clip in `footage/` and track it with DVC (`dvc add ai/datasets/loitering/footage/<clip>.mp4`).
...
```bash
python ai/inference/evaluate_cli.py --category loitering          # this scenario
