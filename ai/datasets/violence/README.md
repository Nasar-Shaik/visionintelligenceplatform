...
Physical altercation between two or more subjects.

## What the runtime produces today

Composite over rapid proximity, velocity and track interaction.

## The honest gap

No pose model is bound. Anything this produces today is inferred from bounding-box dynamics alone.
...
1. Put the clip in `footage/` and track it with DVC (`dvc add ai/datasets/violence/footage/<clip>.mp4`).
...
```bash
python ai/inference/evaluate_cli.py --category violence          # this scenario
