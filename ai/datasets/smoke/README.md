...
Visible smoke, including the slow-building kind that precedes flame.

## What the runtime produces today

`BehaviorResult` from the smoke path; `perception.smoke.detected`.

Event types: `perception.smoke.detected`.

## The honest gap

Smoke against a pale ceiling and steam from a kitchen are the two failure cases; both need footage.
...
1. Put the clip in `footage/` and track it with DVC (`dvc add ai/datasets/smoke/footage/<clip>.mp4`).
...
```bash
python ai/inference/evaluate_cli.py --category smoke          # this scenario
