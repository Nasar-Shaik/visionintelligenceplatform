...
Entry into a zone marked restricted (stockroom, plant room, back-of-house) by someone who should not be there.

## What the runtime produces today

`ZoneTransition` plus an intrusion `BehaviorResult` scoped to the restricted zone.

Event types: `security.intrusion.detected`.

## The honest gap

Who _should_ be there is identity, which the runtime does not have. These cases can only assert that entry was NOTICED.
...
1. Put the clip in `footage/` and track it with DVC (`dvc add ai/datasets/restricted_area/footage/<clip>.mp4`).
...
```bash
python ai/inference/evaluate_cli.py --category restricted_area          # this scenario
