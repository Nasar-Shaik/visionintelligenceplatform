...
Loosely-defined operator intuition: pacing, repeated returns to the same spot, watching staff rather than stock.

## What the runtime produces today

Composites over loitering, re-entry counts and zone dwell.

## The honest gap

The vaguest category in the corpus, and the one most likely to encode bias. Every case must state exactly what a human saw, in behavioural terms.
...
1. Put the clip in `footage/` and track it with DVC (`dvc add ai/datasets/suspicious_behavior/footage/<clip>.mp4`).
...
```bash
python ai/inference/evaluate_cli.py --category suspicious_behavior          # this scenario
