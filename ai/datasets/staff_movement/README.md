...
Where staff are, and separating them from customers.

## What the runtime produces today

The same tracking primitives; the staff/customer distinction is a rule or a uniform-detection model.

## The honest gap

The runtime cannot distinguish staff from customers. Cases must not assume it can.
...
1. Put the clip in `footage/` and track it with DVC (`dvc add ai/datasets/staff_movement/footage/<clip>.mp4`).
...
```bash
python ai/inference/evaluate_cli.py --category staff_movement          # this scenario
