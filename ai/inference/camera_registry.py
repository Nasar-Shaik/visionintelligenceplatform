...
def render_markdown(registry: "CameraRegistry") -> str:
    """The compatibility matrix as a committed document (P-9).

    ⭐ **Generated, never hand-maintained.** A hand-kept support matrix is a document that drifts
    away from the registry the moment someone is in a hurry, and the direction it drifts is always
    optimistic — nobody edits a table to downgrade a device. Deriving it means the only way to change
    what the matrix claims is to change what the evidence says.

    ⚠️ Every status is spelled out in words, for the same reason `render_matrix` does: a tick invites
    an optimistic reading and "Pending Validation" does not.
    """
    entries = registry.entries()
    summary = registry.summary()
    certified = [e for e in entries if e.status == "certified"]
    lines = [
        "<!-- GENERATED by `python certify_cli.py --matrix --markdown`. Do not edit by hand. -->",
        "",
        "# Camera compatibility matrix",
        "",
        f"**{summary['devices']} devices · {len(certified)} certified · "
        f"{summary['byStatus'].get('pending-validation', 0)} pending validation.**",
        "",
    ]
    if not certified:
        lines += [
    ...
    for entry in entries:
        issues = f"{len(entry.known_issues)}" if entry.known_issues else "—"
        lines.append(
            f"| {entry.label} | {entry.kind} | {', '.join(entry.transports) or '—'} | "
            f"{'yes' if entry.onvif else 'no'} | {_title(entry.status)} | "
            f"{entry.evidence_class} | {issues} |"
        )
    # ⚠️ Only when there is something under it. An empty "## Evidence" heading reads as a section
    # someone forgot to fill in, rather than as the accurate statement that no evidence exists yet.
    if any(e.evidence or e.known_issues for e in entries):
        lines += ["", "## Evidence", ""]
    for entry in entries:
        if entry.evidence or entry.known_issues:
            lines.append(f"### {entry.label} (`{entry.id}`)")
            lines.append("")
            if entry.certified_at:
                lines.append(f"- Certified **{entry.certified_at}** · bundle(s): "
                             f"{', '.join(f'`{e}`' for e in entry.evidence)}")
            elif entry.evidence:
                lines.append(f"- Evidence: {', '.join(f'`{e}`' for e in entry.evidence)}")
            for issue in entry.known_issues:
                lines.append(f"- ⚠️ {issue}")
            if entry.recommended_settings:
                lines.append(f"- Recommended: `{json.dumps(entry.recommended_settings, sort_keys=True)}`")
            lines.append("")
    return "\n".join(lines).rstrip() + "\n"
