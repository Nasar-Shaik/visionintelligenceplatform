...
class _Live:
    ...
    def __init__(self, track: Track, history: Deque[TrackHistoryPoint]) -> None:
        ...
        self.misses = 0
        self.transitions: List[dict] = [{"frameIndex": track.first_seen_frame, "from": None, "to": track.state.value}]
...
class TrackManager:
        ...
        *,
        ...
        for di, det in enumerate(detections):
            if di not in matched_dets:
                self._spawn(det, tenant_id, camera_id, frame_index, at)
    ...
    def _transition(self, live: _Live, to: TrackState, frame_index: int) -> None:
        if live.track.state != to:
            live.transitions.append({"frameIndex": frame_index, "from": live.track.state.value, "to": to.value})
            live.track.state = to
    ...
    def _apply_match(self, live: _Live, det: Detection, score: float, frame_index: int, at: str) -> None:
        ...
        self._transition(live, TrackState.CONFIRMED if t.hits >= self._min_hits else TrackState.TENTATIVE, frame_index)
    ...
    def _spawn(self, det: Detection, tenant_id: str, camera_id: str, frame_index: int, at: str) -> None:
        self._seq += 1
